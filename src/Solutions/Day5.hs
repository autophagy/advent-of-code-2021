...
import Common (reversableRange, splitOn)
...
createLine (Coordinate x1 y1) (Coordinate x2 y2)
  | x1 == x2 = map (Coordinate x1) $ reversableRange y1 y2
  | y1 == y2 = map (`Coordinate` y1) $ reversableRange x1 x2
  | otherwise = []
...
createLineDiag (Coordinate x1 y1) (Coordinate x2 y2)
  | x1 == x2 = map (Coordinate x1) $ reversableRange y1 y2
  | y1 == y2 = map (`Coordinate` y1) $ reversableRange x1 x2
  | abs (x1 - x2) == abs (y1 - y2) = zipWith Coordinate (reversableRange x1 x2) (reversableRange y1 y2)
  | otherwise = []
